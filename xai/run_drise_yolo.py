...
# --------------------------------------------
# Reuse utilities from Grad-CAM++ YOLO script
# --------------------------------------------
from run_gradcampp_yolo import (
    load_coco_gt,
...
def load_yolo_model(weights_path: Path) -> YOLO:
    # Ultralytics will choose device at predict-time unless specified
...
def yolo_predict_single(model: YOLO, img_np: np.ndarray):
    ...
    if res.boxes is None:
        ...
        labels = (res.boxes.cls.long().cpu().numpy() + 1).astype(np.int64)
...
def drise_for_target(
    ...
    for i in range(n_samples):
        ...
        boxes_p, scores_p, labels_p = yolo_predict_single(model, masked)
...
def main():
    ...
    for k, img_id in enumerate(sampled_ids, start=1):
        ...
        if gtb.shape[0] > 0:
            mask_top = cam_threshold_top_pct(cam, pct=0.15)
        ...
        if k % 10 == 0 or k == len(sampled_ids):
            print(f"[D-RISE] processed {k}/{len(sampled_ids)}")

    df = pd.DataFrame(rows)
    out_csv = OUT_DIR / "xai_pyro_clean_yolo_drise.csv"
